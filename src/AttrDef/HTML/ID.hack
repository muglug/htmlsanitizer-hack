...
class HTMLPurifier_AttrDef_HTML_ID extends HTMLPurifier\HTMLPurifier_AttrDef {
     ...
     * @return string
     */
    public function validate(string $id, HTMLPurifier\HTMLPurifier_Config $config, HTMLPurifier\HTMLPurifier_Context $context): string {
        ...
        if ($prefix !== '') {
            ...
            //     E_USER_WARNING
            // );
            throw new \Exception('%Attr.IDPrefixLocal cannot be used unless ' .
                    '%Attr.IDPrefix is set');
        }
