 ...
 */
class HTMLPurifier_AttrDef_URI_Host extends HTMLPurifier\HTMLPurifier_AttrDef {

    /**
     * IPv4 sub-validator.
     * @type HTMLPurifier_AttrDef_URI_IPv4
     */
    protected HTMLPurifier_AttrDef_URI_IPv4 $ipv4;

    /**
     * IPv6 sub-validator.
     * @type HTMLPurifier_AttrDef_URI_IPv6
     */
    protected HTMLPurifier_AttrDef_URI_IPv6 $ipv6;
    ...
    public function validate(string $string, HTMLPurifier\HTMLPurifier_Config $config, HTMLPurifier\HTMLPurifier_Context $context): string {
        ...
        if ($length > 1 && $string[0] === '[' && $string[$length - 1] === ']') {
            //IPv6
            $ip = Str\slice($string, 1, $length - 2);
        ...
        $underscore = $config->def->defaults['Core.AllowHostnameUnderscore'] ? '_' : '';
        ...
        $matches = vec[];
        if (\preg_match_with_matches("/^(?:$domainlabel\.)*($toplabel)\.?$/i", $string, inout $matches)) {
        ...
        // since otherwise we have to assume browsers support
            ...
            //         $encodable = false;
            //         for ($i = 0, $c = Str\length($part); $i < $c; $i++) {
            //             if (\ord($part[$i]) > 0x7a) {
            //                 $encodable = true;
            //                 break;
            //             }
