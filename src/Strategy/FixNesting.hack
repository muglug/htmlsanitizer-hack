...
class HTMLPurifier_Strategy_FixNesting extends HTMLPurifier\HTMLPurifier_Strategy {
     ...
     */
    public function stackVal(HTMLPurifier\HTMLPurifier_Node $node, bool $desc, vec<string> $exclude, int $index): shape('top_node' => HTMLPurifier\HTMLPurifier_Node, 'desc' => bool, 'exclude' => vec<string>, 'ix' => int) {
        return shape('top_node' => $node, 'desc' => $desc, 'exclude' => $exclude, 'ix' => $index);
    }

    public function execute(vec<HTMLPurifier\HTMLPurifier_Token> $tokens, HTMLPurifier\HTMLPurifier_Config $config, HTMLPurifier\HTMLPurifier_Context $context): vec<HTMLPurifier\HTMLPurifier_Token> {
        ...
        while (!$stack->isEmpty()) {
            ...
            while ($node is Node\HTMLPurifier_Node_Element && ($ix < C\count($node->children)) && $node->children[$ix] is nonnull) {
                ...
                if ($child is Node\HTMLPurifier_Node_Element) {
                    ...
                    $child_exclude = ($def && $def->excludes is null) ? $excludes : Vec\concat($excludes, $def->excludes);
                    // $stack[] = array($child,
            ...
            if ($excludes_enabled && ($node is Node\HTMLPurifier_Node_Element || $node is Node\HTMLPurifier_Node_Text) && C\contains($excludes, $node->name)) {
                $node->dead = true;
