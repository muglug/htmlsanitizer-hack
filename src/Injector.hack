...
abstract class HTMLPurifier_Injector
{
    ...
    */
    ...
    public function checkNeeded(HTMLPurifier_Config $config) : string {
        ...
        foreach ($this->needed as $element => $attributes) {
            ...
            foreach ($attributes as $name) {
                if ($element is string && C\contains_key($def->info, $element) && !C\contains_key($def->info[$element]->attr, $name)) {
                    return "$element.$name";
                }
            }
            $index++;
    ...
    public function allowsElement(string $name) : bool {
        ...
        if ($parent is nonnull && (!C\contains_key($parent->child->elements, $name) || C\contains($parent->excludes, $name))) {
            return false;
        }
        // check for exclusion
        if (!C\is_empty($this->currentNesting)) {
