...
class HTMLPurifier_Injector_SafeObject extends HTMLPurifier\HTMLPurifier_Injector {
    ...
    public function __construct() : void {
        throw new \Exception("SafeObject is not allowed for the time being. If you are seeing this, please change your AutoFormat configuration.");
    }
    ...
    public function handleElement(HTMLPurifier\HTMLPurifier_Token $token) : mixed {
        ...
        if ($token->name == 'object') {
            ...
            if ($token is Token\HTMLPurifier_Token_Tag && $nest >= 0 && $nestToken->name === 'object') {
                ...
                if (!C\contains_key($objectStackToken->attr, 'data') &&
                ...
                // already been added
                if (!C\contains_key($this->paramStack[$i], $n) &&
